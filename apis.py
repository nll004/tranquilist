...
from secret.keys import quote_API_key
...
def get_user_token(username):
    ...
    # time.
    if os.path.exists(f"secret/{username}token.json"):
        creds = Credentials.from_authorized_user_file(f"secret/{username}token.json", SCOPES)
    ...
    if not creds or not creds.valid:
        ...
        with open(f"secret/{username}token.json", 'w') as token:
